# Claude Code Desktop Detection Test
$nameFragments = @("Claude", "AnthropicClaude")
$excludeFragments = @("CLI", "npm")

Write-Host "=== Claude Code Desktop Detection Test ===" -ForegroundColor Cyan
Write-Host ""
...
foreach ($reg in $roots) {
    ...
    foreach ($item in $items) {
        ...
        # Claude CLI check: AnthropicClaude path
        $isCli = $installLoc -and ($installLoc -match 'AnthropicClaude')

        Write-Host "[1] Registry Detection" -ForegroundColor Yellow
        if ($isCli) {
            Write-Host "    FOUND (but SKIP - CLI detected via path)" -ForegroundColor Magenta
        } else {
...
$pkg = Get-AppxPackage -Name "Anthropic.Claude" -ErrorAction SilentlyContinue
if ($pkg) {
