...
impl ToolPlugin for CodexCliPlugin {
    fn metadata(&self) -> ToolMeta {
        ToolMeta { id: "codex-cli".into(), name: "Codex (CLI)".into(),
            description: "OpenAI 官方 CLI AI 编程助手".into(), icon: "codex".into(), category: "ai-cli".into() }
    }
    ...
    fn detect(&self, install_root: Option<&Path>) -> DetectResult {
        ...
        if let Some(root) = install_root {
            let exe = root.join("codex-cli").join("bin").join("codex.cmd");
            if exe.exists() { return DetectResult { installed: true, version: None, install_path: Some(root.join("codex-cli").to_string_lossy().to_string()) }; }
        }
