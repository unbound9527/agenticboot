...
impl ToolPlugin for OpenCodeDesktopPlugin {
    fn metadata(&self) -> ToolMeta {
        ToolMeta {
            ...
            description: "OpenCode 桌面应用".into(),
            icon: "opencode".into(),
    ...
    fn detect(&self) -> DetectResult {
        ...
        let possible_paths = vec![
            Path::new(&local_app_data).join("Programs").join("OpenCode"),
            Path::new("C:\\Program Files\\OpenCode").to_path_buf(),
        ];
        for p in &possible_paths {
            if p.exists() && p.join("OpenCode.exe").exists() {
                return DetectResult {
                    installed: true,
                    version: None,
    ...
    fn install(&self, _target_dir: &Path, _progress: Sender<InstallProgress>) -> Result<(), String> {
        Err("OpenCode 桌面版需要下载安装包，此功能将在后续版本实现".into())
    }
