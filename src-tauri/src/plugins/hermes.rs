...
impl ToolPlugin for HermesPlugin {
    fn metadata(&self) -> ToolMeta {
        ToolMeta { id: "hermes".into(), name: "Hermes (Web UI)".into(),
            description: "多供应商 AI 编程助手，Web UI 交互".into(), icon: "hermes".into(), category: "ai-cli".into() }
    }
    ...
    fn detect(&self, install_root: Option<&Path>) -> DetectResult {
        ...
        if let Some(root) = install_root {
            let exe = root.join("hermes").join("bin").join("hermes.cmd");
            if exe.exists() { return DetectResult { installed: true, version: None, install_path: Some(root.join("hermes").to_string_lossy().to_string()) }; }
        }
    ...
    fn install(&self, target_dir: &Path, _progress: Sender<InstallProgress>) -> Result<(), String> {
        let output = Command::new("npm").args(["install", "-g", "hermes", "--prefix", &target_dir.to_string_lossy()])
    ...
    fn install(&self, _target_dir: &Path, _progress: Sender<InstallProgress>) -> Result<(), String> {
        Err("Hermes 自动安装目前仅支持 Windows".into())
    }

    fn uninstall(&self, target_dir: &Path) -> Result<(), String> {
        Command::new("npm").args(["uninstall", "-g", "hermes", "--prefix", &target_dir.to_string_lossy()])
